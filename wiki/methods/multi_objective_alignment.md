...
- 对齐粒度演进：token-level → embedding-level → task-level → list-level → business-level routing → **outcome-conditioned probability shaping**
...
### 对齐范式对比：列表级 vs 业务级 vs 结果条件化

| 维度 | HiGR（列表级对齐） | MBGR（业务级动态路由） | PinRec（结果条件化生成） |
|------|-------------------|------------------------|--------------------------|
| **优化粒度** | Slate/列表级联合打分 | Token/Step级业务条件路由 | Token/Step级概率分布重塑 |
| **目标解耦方式** | 损失函数加权（α, β, γ） | 业务专属预测分支 + 门控激活 | 条件信号注入 + 动态概率调制 |
| **标签/信号处理** | 隐式反馈直接建模 | 稀疏标签动态稠密化（LDR） | 业务权重作为生成先验条件 |
| **适用场景** | 单业务/垂直内容推荐 | 超级App多业务线统一生成 | 工业级大规模检索/召回 |
| **核心优势** | 列表多样性与用户体验平衡 | 缓解跷跷板效应，跨业务协同生成 | 免重训动态调权，多Token提效增样 |
...
- 通过直接修改 Softmax 前的 Logits 分布，实现**免重训的动态多目标对齐**，彻底规避传统加权损失中的梯度冲突与超参敏感问题。
...
- **训练策略**：多业务混合采样 + 梯度裁剪 + 标签平滑稠密化 + 条件分布正则，保障 Scaling Law 下的训练稳定性。
...
| 业务目标 | 留存率, 转化率, 收入/GMV, 跷跷板效应抑制率, 策略调权响应延迟 |
| 公平性 | 物品曝光分布, 创作者/商户公平性, 长尾业务覆盖率 |
| 生成质量 | Recall@K, NDCG@K, 序列连贯性, 推理延迟(P99), 吞吐(TPS) |
...
9. **多Token解码的可控性边界**：并行预测多个Token时，如何保证跨步目标一致性（如避免前一个Token优化CTR而后一个Token破坏多样性）？
...
**来源**: [2504_paper_25041050_PinRec_Outcome-Conditioned,_Multi-Token_Generative_Retrieva.md](../sources/2504_paper_25041050_PinRec_Outcome-Conditioned,_Multi-Token_Generative_Retrieva.md)
- 新增 **PinRec 的结果条件化生成与多Token解码** 章节，详细阐述如何通过条件信号动态调整生成概率分布以替代传统加权损失，实现免重训的多目标动态权衡。
- 扩展对齐范式对比表与分类表，纳入 Outcome-Conditioned 范式，明确其在工业级检索场景中的定位与优势。
- 更新技术实现细节，补充条件化 Logit 调制公式与多Token并行解码机制，完善评估指标体系（新增保存率、长尾覆盖率、策略调权响应延迟等）。
- 补充条件化生成分布稳定性与多Token可控性相关的 Open Questions。

---

## 更新完成：2504_paper_25041050_PinRec_Outcome-Conditioned,_Multi-Token_Generative_Retrieva.md
**更新时间**: 2026-04-13 14:20
**更新摘要**: 已使用 LLM 对页面进行内容充实，基于 PinRec 论文引入结果条件化生成范式，完善多目标对齐在生成式检索中的技术路径与工业实践对比。
